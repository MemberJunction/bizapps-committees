...
export class mjCommitteesCommitteeResolver extends ResolverBase {
    ...
    async mjCommitteesMemberships_CommitteeIDArray(@Root() mjcommitteescommittee_: mjCommitteesCommittee_, @Ctx() { userPayload, providers }: AppContext, @PubSub() pubSub: PubSubEngine) {
        ...
        const provider = GetReadOnlyProvider(providers, { allowFallbackToReadWrite: true });
        const sSQL = `SELECT * FROM ${provider.QuoteSchemaAndView('__mj_Committees', 'vwMemberships')} WHERE ${provider.QuoteIdentifier('CommitteeID')}='${mjcommitteescommittee_.ID}' ` + this.getRowLevelSecurityWhereClause(provider, 'Memberships', userPayload, EntityPermissionType.Read, 'AND');
        const rows = await provider.ExecuteSQL(sSQL, undefined, undefined, this.GetUserFromPayload(userPayload));
...
export class mjBizAppsTasksTask_ {
    ...
    @Field(() => [mjCommitteesComment_])
    mjCommitteesComments_TaskIDArray: mjCommitteesComment_[]; // Link to mjCommitteesComments
    ...
    @Field(() => [mjCommitteesArtifact_])
    mjCommitteesArtifacts_TaskIDArray: mjCommitteesArtifact_[]; // Link to mjCommitteesArtifacts
...
export class mjBizAppsTasksTaskResolver extends ResolverBase {
    ...
    @FieldResolver(() => [mjCommitteesComment_])
    async mjCommitteesComments_TaskIDArray(@Root() mjbizappstaskstask_: mjBizAppsTasksTask_, @Ctx() { userPayload, providers }: AppContext, @PubSub() pubSub: PubSubEngine) {
        this.CheckUserReadPermissions('Comments', userPayload);
        const provider = GetReadOnlyProvider(providers, { allowFallbackToReadWrite: true });
        const sSQL = `SELECT * FROM ${provider.QuoteSchemaAndView('__mj_Committees', 'vwComments')} WHERE ${provider.QuoteIdentifier('TaskID')}='${mjbizappstaskstask_.ID}' ` + this.getRowLevelSecurityWhereClause(provider, 'Comments', userPayload, EntityPermissionType.Read, 'AND');
        const rows = await provider.ExecuteSQL(sSQL, undefined, undefined, this.GetUserFromPayload(userPayload));
        const result = await this.ArrayMapFieldNamesToCodeNames('Comments', rows, this.GetUserFromPayload(userPayload));
        return result;
    }
    ...
    @FieldResolver(() => [mjCommitteesArtifact_])
    async mjCommitteesArtifacts_TaskIDArray(@Root() mjbizappstaskstask_: mjBizAppsTasksTask_, @Ctx() { userPayload, providers }: AppContext, @PubSub() pubSub: PubSubEngine) {
        this.CheckUserReadPermissions('Artifacts', userPayload);
        const provider = GetReadOnlyProvider(providers, { allowFallbackToReadWrite: true });
        const sSQL = `SELECT * FROM ${provider.QuoteSchemaAndView('__mj_Committees', 'vwArtifacts')} WHERE ${provider.QuoteIdentifier('TaskID')}='${mjbizappstaskstask_.ID}' ` + this.getRowLevelSecurityWhereClause(provider, 'Artifacts', userPayload, EntityPermissionType.Read, 'AND');
        const rows = await provider.ExecuteSQL(sSQL, undefined, undefined, this.GetUserFromPayload(userPayload));
        const result = await this.ArrayMapFieldNamesToCodeNames('Artifacts', rows, this.GetUserFromPayload(userPayload));
        return result;
    }
