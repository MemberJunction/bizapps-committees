...
--   2. APPLICATIONS       — both Explorer apps, correctly named up front;
--                           CodeGen attaches every generated entity to them
--   3. CODEGEN section    — spliced in after running `mj codegen` on a fresh DB:
--                           entity registrations, audit columns, FK indexes,
--                           base views, CRUD procs, triggers, permissions,
--                           ApplicationEntity rows for the two apps above
-- Seed/lookup data is NOT here — it lives in metadata/ via `mj sync push`.
-- On schema changes: edit the DDL, wipe + re-run codegen on a fresh DB, replace the CODEGEN section.
--
...
CREATE TABLE ${flyway:defaultSchema}.Motion (
    ID UNIQUEIDENTIFIER NOT NULL DEFAULT NEWSEQUENTIALID(),
    MeetingID UNIQUEIDENTIFIER,  -- NULL for between-meeting e-ballot motions (see Ballot)
    AgendaItemID UNIQUEIDENTIFIER,
...
---------------------------------------------------------------------------
-- Ballot — one voting window per motion
---------------------------------------------------------------------------
IF NOT EXISTS (SELECT 1 FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA = '${flyway:defaultSchema}' AND TABLE_NAME = 'Ballot')
CREATE TABLE ${flyway:defaultSchema}.Ballot (
    ID UNIQUEIDENTIFIER NOT NULL DEFAULT NEWSEQUENTIALID(),
    CommitteeID UNIQUEIDENTIFIER NOT NULL,
    MotionID UNIQUEIDENTIFIER NOT NULL,
    OpensAt DATETIMEOFFSET NOT NULL,
    ClosesAt DATETIMEOFFSET NOT NULL,
    ClosedAt DATETIMEOFFSET,
    ThresholdType NVARCHAR(20) NOT NULL DEFAULT 'SimpleMajority',
    IsSealed BIT NOT NULL DEFAULT 1,
    Status NVARCHAR(20) NOT NULL DEFAULT 'Open',
    CreatedByMembershipID UNIQUEIDENTIFIER,
    ResultNotes NVARCHAR(MAX),
    CONSTRAINT PK_Ballot PRIMARY KEY (ID),
    CONSTRAINT FK_Ballot_Committee FOREIGN KEY (CommitteeID) REFERENCES ${flyway:defaultSchema}.Committee(ID),
    CONSTRAINT FK_Ballot_Motion FOREIGN KEY (MotionID) REFERENCES ${flyway:defaultSchema}.Motion(ID),
    CONSTRAINT FK_Ballot_CreatedBy FOREIGN KEY (CreatedByMembershipID) REFERENCES ${flyway:defaultSchema}.Membership(ID),
    CONSTRAINT UQ_Ballot_Motion UNIQUE (MotionID),
    CONSTRAINT CK_Ballot_ThresholdType CHECK (ThresholdType IN ('SimpleMajority', 'TwoThirds', 'Unanimous')),
    CONSTRAINT CK_Ballot_Status CHECK (Status IN ('Open', 'Closed', 'Cancelled')),
    CONSTRAINT CK_Ballot_Window CHECK (ClosesAt > OpensAt)
);
GO
...
-- Ballot (e-ballot voting window) + Motion.MeetingID nullability
IF NOT EXISTS (SELECT 1 FROM sys.extended_properties WHERE major_id = OBJECT_ID('${flyway:defaultSchema}.Ballot') AND minor_id = 0 AND name = 'MS_Description')
    EXEC sp_addextendedproperty @name = N'MS_Description', @value = N'Between-meeting e-ballot: the voting window for exactly one Motion, with pass threshold and sealed-choice flag. Votes attach to the Motion as ordinary Vote records; while sealed, vote choices are withheld from all consumers until the ballot closes.', @level0type = N'SCHEMA', @level0name = N'${flyway:defaultSchema}', @level1type = N'TABLE', @level1name = N'Ballot';
GO

IF NOT EXISTS (SELECT 1 FROM sys.extended_properties WHERE major_id = OBJECT_ID('${flyway:defaultSchema}.Ballot') AND minor_id = COLUMNPROPERTY(OBJECT_ID('${flyway:defaultSchema}.Ballot'), 'CommitteeID', 'ColumnId') AND name = 'MS_Description')
    EXEC sp_addextendedproperty @name = N'MS_Description', @value = N'Committee this ballot belongs to; also the committee scope for the ballot''s meeting-less Motion', @level0type = N'SCHEMA', @level0name = N'${flyway:defaultSchema}', @level1type = N'TABLE', @level1name = N'Ballot', @level2type = N'COLUMN', @level2name = N'CommitteeID';
GO

IF NOT EXISTS (SELECT 1 FROM sys.extended_properties WHERE major_id = OBJECT_ID('${flyway:defaultSchema}.Ballot') AND minor_id = COLUMNPROPERTY(OBJECT_ID('${flyway:defaultSchema}.Ballot'), 'MotionID', 'ColumnId') AND name = 'MS_Description')
    EXEC sp_addextendedproperty @name = N'MS_Description', @value = N'The Motion this ballot decides — exactly one ballot per motion (unique)', @level0type = N'SCHEMA', @level0name = N'${flyway:defaultSchema}', @level1type = N'TABLE', @level1name = N'Ballot', @level2type = N'COLUMN', @level2name = N'MotionID';
GO

IF NOT EXISTS (SELECT 1 FROM sys.extended_properties WHERE major_id = OBJECT_ID('${flyway:defaultSchema}.Ballot') AND minor_id = COLUMNPROPERTY(OBJECT_ID('${flyway:defaultSchema}.Ballot'), 'OpensAt', 'ColumnId') AND name = 'MS_Description')
    EXEC sp_addextendedproperty @name = N'MS_Description', @value = N'When voting opens', @level0type = N'SCHEMA', @level0name = N'${flyway:defaultSchema}', @level1type = N'TABLE', @level1name = N'Ballot', @level2type = N'COLUMN', @level2name = N'OpensAt';
GO

IF NOT EXISTS (SELECT 1 FROM sys.extended_properties WHERE major_id = OBJECT_ID('${flyway:defaultSchema}.Ballot') AND minor_id = COLUMNPROPERTY(OBJECT_ID('${flyway:defaultSchema}.Ballot'), 'ClosesAt', 'ColumnId') AND name = 'MS_Description')
    EXEC sp_addextendedproperty @name = N'MS_Description', @value = N'Scheduled close of the voting window (UI countdown target)', @level0type = N'SCHEMA', @level0name = N'${flyway:defaultSchema}', @level1type = N'TABLE', @level1name = N'Ballot', @level2type = N'COLUMN', @level2name = N'ClosesAt';
GO

IF NOT EXISTS (SELECT 1 FROM sys.extended_properties WHERE major_id = OBJECT_ID('${flyway:defaultSchema}.Ballot') AND minor_id = COLUMNPROPERTY(OBJECT_ID('${flyway:defaultSchema}.Ballot'), 'ClosedAt', 'ColumnId') AND name = 'MS_Description')
    EXEC sp_addextendedproperty @name = N'MS_Description', @value = N'When the ballot actually closed (early close or scheduled); NULL while open', @level0type = N'SCHEMA', @level0name = N'${flyway:defaultSchema}', @level1type = N'TABLE', @level1name = N'Ballot', @level2type = N'COLUMN', @level2name = N'ClosedAt';
GO

IF NOT EXISTS (SELECT 1 FROM sys.extended_properties WHERE major_id = OBJECT_ID('${flyway:defaultSchema}.Ballot') AND minor_id = COLUMNPROPERTY(OBJECT_ID('${flyway:defaultSchema}.Ballot'), 'ThresholdType', 'ColumnId') AND name = 'MS_Description')
    EXEC sp_addextendedproperty @name = N'MS_Description', @value = N'Pass threshold measured against the committee''s voting members: SimpleMajority, TwoThirds, or Unanimous', @level0type = N'SCHEMA', @level0name = N'${flyway:defaultSchema}', @level1type = N'TABLE', @level1name = N'Ballot', @level2type = N'COLUMN', @level2name = N'ThresholdType';
GO

IF NOT EXISTS (SELECT 1 FROM sys.extended_properties WHERE major_id = OBJECT_ID('${flyway:defaultSchema}.Ballot') AND minor_id = COLUMNPROPERTY(OBJECT_ID('${flyway:defaultSchema}.Ballot'), 'IsSealed', 'ColumnId') AND name = 'MS_Description')
    EXEC sp_addextendedproperty @name = N'MS_Description', @value = N'When 1, individual vote choices are withheld from all consumers (including admins) until the ballot closes; participation (who has voted) remains visible', @level0type = N'SCHEMA', @level0name = N'${flyway:defaultSchema}', @level1type = N'TABLE', @level1name = N'Ballot', @level2type = N'COLUMN', @level2name = N'IsSealed';
GO

IF NOT EXISTS (SELECT 1 FROM sys.extended_properties WHERE major_id = OBJECT_ID('${flyway:defaultSchema}.Ballot') AND minor_id = COLUMNPROPERTY(OBJECT_ID('${flyway:defaultSchema}.Ballot'), 'Status', 'ColumnId') AND name = 'MS_Description')
    EXEC sp_addextendedproperty @name = N'MS_Description', @value = N'Ballot lifecycle: Open (accepting votes), Closed (result computed, votes unsealed, Motion stamped), Cancelled', @level0type = N'SCHEMA', @level0name = N'${flyway:defaultSchema}', @level1type = N'TABLE', @level1name = N'Ballot', @level2type = N'COLUMN', @level2name = N'Status';
GO

IF NOT EXISTS (SELECT 1 FROM sys.extended_properties WHERE major_id = OBJECT_ID('${flyway:defaultSchema}.Ballot') AND minor_id = COLUMNPROPERTY(OBJECT_ID('${flyway:defaultSchema}.Ballot'), 'CreatedByMembershipID', 'ColumnId') AND name = 'MS_Description')
    EXEC sp_addextendedproperty @name = N'MS_Description', @value = N'Membership of the member who opened the ballot (typically the Chair)', @level0type = N'SCHEMA', @level0name = N'${flyway:defaultSchema}', @level1type = N'TABLE', @level1name = N'Ballot', @level2type = N'COLUMN', @level2name = N'CreatedByMembershipID';
GO

IF NOT EXISTS (SELECT 1 FROM sys.extended_properties WHERE major_id = OBJECT_ID('${flyway:defaultSchema}.Ballot') AND minor_id = COLUMNPROPERTY(OBJECT_ID('${flyway:defaultSchema}.Ballot'), 'ResultNotes', 'ColumnId') AND name = 'MS_Description')
    EXEC sp_addextendedproperty @name = N'MS_Description', @value = N'Certification text recorded at close (result summary, any procedural notes)', @level0type = N'SCHEMA', @level0name = N'${flyway:defaultSchema}', @level1type = N'TABLE', @level1name = N'Ballot', @level2type = N'COLUMN', @level2name = N'ResultNotes';
GO

IF NOT EXISTS (SELECT 1 FROM sys.extended_properties WHERE major_id = OBJECT_ID('${flyway:defaultSchema}.Motion') AND minor_id = COLUMNPROPERTY(OBJECT_ID('${flyway:defaultSchema}.Motion'), 'MeetingID', 'ColumnId') AND name = 'MS_Description')
    EXEC sp_addextendedproperty @name = N'MS_Description', @value = N'Meeting where the motion was made; NULL for between-meeting e-ballot motions (see Ballot)', @level0type = N'SCHEMA', @level0name = N'${flyway:defaultSchema}', @level1type = N'TABLE', @level1name = N'Motion', @level2type = N'COLUMN', @level2name = N'MeetingID';
GO

---------------------------------------------------------------------------
-- APPLICATIONS — created here, correctly named, BEFORE codegen ever runs.
--
-- CodeGen only auto-creates a schema-named app when NO Application row has
-- SchemaAutoAddNewEntities covering the schema (manage-metadata.js:
-- getApplicationIDForSchema). Because both apps below claim the schema,
-- CodeGen skips its auto-create and instead attaches every generated entity
-- to BOTH apps automatically — no rename, no entity-copy, no ordering trap.
--
-- Two-app architecture:
--   Committees            (mjcommittees)     — member/officer participation app
--   Committee Management  (mjcommitteemgmt)  — staff-facing administrative app
---------------------------------------------------------------------------
IF NOT EXISTS (SELECT 1 FROM ${mjSchema}.Application WHERE ID = 'B5D2C7A4-3E61-4E8B-9F2A-6C815D9E4A01')
    INSERT INTO ${mjSchema}.Application (ID, Name, Description, Icon, Color, Path, AutoUpdatePath, SchemaAutoAddNewEntities, DefaultNavItems)
    VALUES (
        'B5D2C7A4-3E61-4E8B-9F2A-6C815D9E4A01',
        'Committees',
        'Member and officer participation app for committees, meetings, voting, and task tracking',
        'fa-solid fa-people-group',
        '#0076b6',
        'mjcommittees',
        0,
        '${flyway:defaultSchema}',
        '[{"Label":"My Committees","Icon":"fa-solid fa-user","ResourceType":"Custom","DriverClass":"MemberHomeComponent","isDefault":true},{"Label":"Committees","Icon":"fa-solid fa-users-rectangle","ResourceType":"Custom","DriverClass":"CommitteeListComponent","isDefault":false},{"Label":"Members","Icon":"fa-solid fa-user-group","ResourceType":"Custom","DriverClass":"MembershipListComponent","isDefault":false},{"Label":"Meetings","Icon":"fa-solid fa-calendar-days","ResourceType":"Custom","DriverClass":"MeetingListComponent","isDefault":false},{"Label":"Action Items","Icon":"fa-solid fa-list-check","ResourceType":"Custom","DriverClass":"ActionItemTrackerComponent","isDefault":false},{"Label":"Documents","Icon":"fa-solid fa-file-lines","ResourceType":"Custom","DriverClass":"DocumentBrowserComponent","isDefault":false}]'
    );
GO

IF NOT EXISTS (SELECT 1 FROM ${mjSchema}.Application WHERE ID = 'B5D2C7A4-3E61-4E8B-9F2A-6C815D9E4A02')
    INSERT INTO ${mjSchema}.Application (ID, Name, Description, Icon, Color, Path, AutoUpdatePath, SchemaAutoAddNewEntities, DefaultNavItems)
    VALUES (
        'B5D2C7A4-3E61-4E8B-9F2A-6C815D9E4A02',
        ...
        0,
        '${flyway:defaultSchema}',
        '[{"Label":"Command Center","Icon":"fa-solid fa-chart-simple","ResourceType":"Custom","DriverClass":"CommandCenterComponent","isDefault":true},{"Label":"Committees","Icon":"fa-solid fa-users-rectangle","ResourceType":"Custom","DriverClass":"ManagementCommitteeListComponent","isDefault":false},{"Label":"Members","Icon":"fa-solid fa-user-group","ResourceType":"Custom","DriverClass":"ManagementMembershipListComponent","isDefault":false},{"Label":"People & Terms","Icon":"fa-solid fa-timeline","ResourceType":"Custom","DriverClass":"PeopleTermsComponent","isDefault":false},{"Label":"Meetings","Icon":"fa-solid fa-calendar-days","ResourceType":"Custom","DriverClass":"MeetingListComponent","isDefault":false},{"Label":"Motions","Icon":"fa-solid fa-gavel","ResourceType":"Custom","DriverClass":"MotionsBallotsComponent","isDefault":false},{"Label":"Actions","Icon":"fa-solid fa-list-check","ResourceType":"Custom","DriverClass":"ActionItemTrackerComponent","isDefault":false},{"Label":"Documents","Icon":"fa-solid fa-file-lines","ResourceType":"Custom","DriverClass":"DocumentBrowserComponent","isDefault":false}]'
    );
GO

-- Role grants CodeGen would have added had it created the app itself
-- (newSchemaDefaults.ApplicationRoleDefaults: UI/Integration access, Developer admin)
INSERT INTO ${mjSchema}.ApplicationRole (ApplicationID, RoleID, CanAccess, CanAdmin)
SELECT a.ID, r.ID,
       1,
       CASE WHEN r.Name = 'Developer' THEN 1 ELSE 0 END
FROM ${mjSchema}.Application a
CROSS JOIN ${mjSchema}.[Role] r
WHERE a.ID IN ('B5D2C7A4-3E61-4E8B-9F2A-6C815D9E4A01', 'B5D2C7A4-3E61-4E8B-9F2A-6C815D9E4A02')
  AND r.Name IN ('UI', 'Developer', 'Integration')
  AND NOT EXISTS (
      SELECT 1 FROM ${mjSchema}.ApplicationRole x
      WHERE x.ApplicationID = a.ID AND x.RoleID = r.ID
  );
GO

/*================================ CODEGEN SPLICE POINT ================================
  Run `npm run mj:migrate` against a FRESH database, then `npm run mj:codegen` (single
  pass — SmartFieldIdentification is pinned OFF in mj.config.cjs; entities use literal
  Name columns so name-field selection is deterministic). Splice the emitted SQL here,
  at the END of this file. The APPLICATIONS section above already created both apps,
  so codegen attaches entities to them and never auto-creates a schema-named app.
  After splicing, verify zero drift:
    SELECT e.Name, c.COLUMN_NAME FROM __mj.Entity e
    ...
    WHERE e.SchemaName = '__mj_BizAppsCommittees' AND ef.ID IS NULL;
=======================================================================================*/

/*  CODEGEN — output of `mj codegen` (@memberjunction/cli 5.44.0), SINGLE PASS,
    run 2026-07-06 against a fresh DB (COMM_DONE) after the sections above.
    Verified: drift query = 0 rows; both apps hold all 17 entities.  */
