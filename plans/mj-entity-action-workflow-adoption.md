...
## 2. What this means for BizApps Committees

Committee work is dense with lifecycle moments that currently need a person to remember something:
a term starting, a term ending, a seat going vacant, quorum lapsing. Those are hook points.

Scope binding is a good fit because committees are **typed** — a governance board, a technical
working group and a nominating committee have genuinely different onboarding and offboarding
workflows, and today that difference has nowhere to live except code or convention.

## 3. Suggested bindings

| Entity + invocation | Scope | Work | Purpose |
|---|---|---|---|
| Committee membership · `AfterCreate` | a committee **type** | Flow agent | Onboarding — welcome, orientation materials, access grants, calendar invites |
| Committee membership · `AfterUpdate` (term ended) | a committee type | Flow agent | Offboarding — access revocation, thank-you, alumni status |
| Committee · `AfterUpdate` (status changed) | a committee type | Action | Notify staff liaison and members |
| Membership · `Validate` | a committee type | Action | Eligibility rules — term limits, concurrent-service caps, conflict-of-interest checks |

## 4. Notes specific to this repo

**The `Validate` gate is the interesting one here.** Committee eligibility rules — term limits,
maximum concurrent appointments, cooling-off periods — are exactly the kind of policy that varies
per organization and per committee type, and that is painful to encode in application logic. A
`Validate` binding scoped to a committee type makes it configuration, and it *actually blocks* the
save rather than warning after the fact.

**Committees is named in the portfolio as an MJ Central anchor app and an open-core candidate.**
That raises the value of configuration-over-code here specifically: an adopter who can change the
onboarding workflow without forking is a materially better story than one who cannot.

**Panels are Committees** in the ethics and awards designs, so any workflow authored here is
reusable by those apps through the same mechanism — another reason to put the policy in bindings
rather than in Committees' own code.
